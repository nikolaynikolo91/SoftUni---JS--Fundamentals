function browserHistory(input) {
    ...
    for (const grade of sortedGrade) {
        let students = objRegister[grade];

        let listStudents = students.map((x) => x.name).join(', ');
        let allGrades = students.map((x) => x.score);
        let average = allGrades.reduce((a, b) => a + b, 0) / students.length;
...
browserHistory(["Student name: Mark, Grade: 8, Graduated with an average score: 4.75",
