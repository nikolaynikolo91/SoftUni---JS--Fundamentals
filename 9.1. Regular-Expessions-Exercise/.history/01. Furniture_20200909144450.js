function solve(input) {
    ...
    let pattern4 = />>(?<name>[\w]+)<<(?<price>[\d]+[\.]?\d+?)\!(?<quantity>[\d]+)gmi/;
    let pattern5 = new RegExp('>>(?<name>[\w]+)<<(?<price>[\d]+[\.]?\d+?)\!(?<quantity>[\d]+)', g)
    ...
    for (const line of input) {
        ...
        // let exec = pattern5.test(line)
        let match = RegExp.match(line, pattern4)
        console.log(match)
    }
